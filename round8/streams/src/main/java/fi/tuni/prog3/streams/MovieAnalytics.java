...
public class MovieAnalytics {
    ...
    public Stream<Movie> moviesAfter(int year){
        return movies.stream().filter(movie -> movie.getReleaseYear() >= year).sorted(Movie.RELEASE_YEAR_CMP).sorted(Movie::compareTo);
    }
    ...
    public Stream<Movie> moviesBetween(int yearA, int yearB){
        return movies.stream().filter(movie -> movie.getReleaseYear() >= yearA && movie.getReleaseYear() <= yearB).sorted(Movie.RELEASE_YEAR_CMP).sorted(Movie::compareTo);
    }

    public Stream<Movie> moviesByDirector(String director){
        return movies.stream().filter(movie -> movie.getDirector().equals(director)).sorted(Movie.RELEASE_YEAR_CMP).sorted(Movie::compareTo);
    }
